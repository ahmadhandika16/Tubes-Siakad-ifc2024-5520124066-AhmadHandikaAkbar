...
<div class="bg-white border border-gray-300 p-4 max-w-xl">
    <form action="{{ route('admin.krs.update', $krs->id) }}" method="POST" class="space-y-3">
        @csrf @method('PUT')
        <div>
            ...
            <select name="npm" id="npm" required
                ...
                @foreach($mahasiswaList as $m)
                    <option value="{{ $m->npm }}" {{ old('npm', $krs->npm) == $m->npm ? 'selected' : '' }}>{{ $m->nama }} ({{ $m->npm }})</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select name="kode_matakuliah" id="kode_matakuliah" required
                ...
                @foreach($matakuliahList as $mk)
                    <option value="{{ $mk->kode_matakuliah }}" {{ old('kode_matakuliah', $krs->kode_matakuliah) == $mk->kode_matakuliah ? 'selected' : '' }}>
        ...
        <div>
            ...
            <input type="text" name="semester" id="semester" value="{{ old('semester', $krs->semester) }}" maxlength="20" required
